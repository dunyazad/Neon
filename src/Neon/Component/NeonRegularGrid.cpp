...
namespace Neon
{
		...
		: ComponentBase(name), mesh(mesh), cellSize(cellSize), cellHalfSize(cellSize * 0.5f)
	{
		...
		cellCountX = (size_t)ceilf(meshAABB.GetXLength() / cellSize);
		cellCountY = (size_t)ceilf(meshAABB.GetYLength() / cellSize);
		cellCountZ = (size_t)ceilf(meshAABB.GetZLength() / cellSize);
	}
	...
	tuple<int, int, int> RegularGrid::InsertPoint(const glm::vec3& position)
	{
		...
		if (nullptr == cell) {
			...
			auto newCell = new RegularGridCell<Vertex, Triangle>(index, glm::vec3(minx, miny, minz), glm::vec3(maxx, maxy, maxz));
			cells[index] = newCell;
			Expand(position);
			return index;
		}
	...
	tuple<int, int, int> RegularGrid::InsertVertex(Vertex* vertex)
	{
				...
				for (int x = get<0>(minIndex); x <= get<0>(maxIndex); x++) {
					...
					if (aabb.IntersectsTriangle(p0, p1, p2)) {
						...
						if (cells.count(index) == 0) {
							auto newCell = new RegularGridCell<Vertex, Triangle>(index, aabb.GetMinPoint(), aabb.GetMaxPoint());
