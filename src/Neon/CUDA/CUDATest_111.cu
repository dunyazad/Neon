...
namespace NeonCUDA
{
	...
	__device__ void GetSuperTriangle(Eigen::Vector3f* points, size_t pointsSize, Triangle* triangles, int* trianglesSize)
	{
		...
		printf("%.6f, %.6f, %.6f\n",
			points[pointsSize - 3 + 0].x(),
			points[pointsSize - 3 + 0].y(),
			points[pointsSize - 3 + 0].z());
		printf("%.6f, %.6f, %.6f\n",
			points[pointsSize - 3 + 1].x(),
			points[pointsSize - 3 + 1].y(),
			points[pointsSize - 3 + 1].z());
		printf("%.6f, %.6f, %.6f\n",
			points[pointsSize - 3 + 2].x(),
			points[pointsSize - 3 + 2].y(),
			points[pointsSize - 3 + 2].z());
