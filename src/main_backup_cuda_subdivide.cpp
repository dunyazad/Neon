...
#pragma endregion
		...
		*/

		{
			...
			NeonCUDA::CUDAMesh cm;
			{
				auto nov = mesh->GetVertexBuffer()->Size();
				for (size_t i = 0; i < nov; i++)
				{
					cm.AddVertex(mesh->GetVertex(i));
				}

				auto noi = mesh->GetIndexBuffer()->Size();
				for (size_t i = 0; i < noi; i++)
				{
					cm.AddIndex(mesh->GetIndex(i));
				}
			}
			...
			{
				mesh->Clear();
				for (auto& v : cm.host_vertices)
				{
					mesh->AddVertex(v);
				}
				for (auto& i : cm.host_indices)
				{
					mesh->AddIndex(i);

					//if (i > mesh->GetVertexBuffer()->Size())
					//{
					//	cout << "!!!!!!!!!!!!!!!!" << endl;
					//}
				}

				mesh->FillColor(glm::vec4(1.0f, 0.0f, 0.0f, 1.0f));
				mesh->RecalculateFaceNormal();
			}
